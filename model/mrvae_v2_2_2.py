...
'''
    原始的mrvae07 的结构，现在添加对每个卷积模块特征图的输出
'''
...
def encoder(inputs, scope):
    ...
    with tf.variable_scope('encoder',reuse=tf.AUTO_REUSE):
        ...
        # 最后一层，计算均值，不需要池化
        encoder_conv51 = encoder_block(name='z_mean', inputs=max_pool4, num_outputs=cfg_v2_2.latent_dim,is_pool=False)      ;tf.add_to_collection('feature_maps',encoder_conv51)
        encoder_conv52 = encoder_block(name='z_variance', inputs=max_pool4, num_outputs=cfg_v2_2.latent_dim,is_pool=False)  ;tf.add_to_collection('feature_maps',encoder_conv52)
...
def GR(inputs, conv1, conv2, conv3, conv4, up_w, up_h):
    ...
    with tf.variable_scope('decoder_first'):
        with slim.arg_scope([slim.conv2d],
            ...
            '''上采样'''
            up_w *= 2; up_h *= 2
            GR_upsampling1 = decoder_block(name='upsampling1', inputs=inputs, num_outputs=cfg_v2_2.latent_dim,num_outputs_up=512, is_resize=True, img_width=up_w, img_height=up_h)
            tf.add_to_collection('feature_maps',GR_upsampling1)

            up_w *= 2; up_h *= 2
            GR_upsampling2 = decoder_block(name='upsampling2', inputs=GR_upsampling1, num_outputs=512,num_outputs_up=256, en_conv=conv4, is_resize=True, img_width=up_w,img_height=up_h)
            tf.add_to_collection('feature_maps', GR_upsampling2)

            up_w *= 2; up_h *= 2
            GR_upsampling3 = decoder_block(name='upsampling3', inputs=GR_upsampling2, num_outputs=256,num_outputs_up=128, en_conv=conv3, is_resize=True, img_width=up_w,img_height=up_h)
            tf.add_to_collection('feature_maps', GR_upsampling3)

            up_w *= 2; up_h *= 2
            GR_upsampling4 = decoder_block(name='upsampling4', inputs=GR_upsampling3, num_outputs=128,num_outputs_up=64,en_conv=conv2, is_resize=True, img_width=up_w, img_height=up_h)
...
def residual_block(no, inputs, inputs64=None):
    ...
    with tf.variable_scope(name):
        with slim.arg_scope([slim.conv2d],
            ...
            r = 16  # 降维比例，需要多次调试
            global_pooling = tf.nn.avg_pool(value=residual1_conv2_BN, ksize=[1, 512, 512, 1], strides=[1, 512, 512, 1],
                                            padding='SAME')
...
def DR(inputs, de_conv1, is_training=True):
    ...
    with tf.variable_scope('decoder_second'):
        with slim.arg_scope([slim.conv2d],
            ...
            DR_out67 = slim.conv2d(DR_out64_prelu, num_outputs=inputs_concat_channels, kernel_size=1,
                                scope='DR_out67')  # 调整通道数
...
def mrvae():
    t_image_ph = tf.placeholder(name='train_images', dtype=tf.float32,
                                shape=[cfg_v2_2.batch_size, cfg_v2_2.image_size, cfg_v2_2.image_size, 3])
    t_org_image_ph = tf.placeholder(name='train_images', dtype=tf.float32,
                                    shape=[cfg_v2_2.batch_size, cfg_v2_2.image_size, cfg_v2_2.image_size, 3])

    # 搭建mrvae
    with tf.name_scope(cfg_v2_2.name):
    ...
    with tf.name_scope('Loss'):
        ...
        # train_op = slim.train.AdamOptimizer(cfg_v2_2.lr).minimize(vae_loss)
        global_step, train_step, lr  = dk.set_optimizer(lr_range=cfg_v2_2.lr_range,num_batches_per_epoch=cfg_v2_2.lr_decay_batches,loss=vae_loss)
