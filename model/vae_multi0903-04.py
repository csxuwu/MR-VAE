...
class multi_vae():
    ...
    def train_vae_multi(self):
        ...
        with tf.name_scope('Loss'):
            ...
            vae_loss = (12*L2 + L1 + kl_loss + content_loss)
    ...
    def test(self):
        ...
        with tf.Session() as sess:
            ...
            # model_saver.restore(sess, '138600.ckpt')
            print('------ 已加载模型 ------')
            writer = tf.summary.FileWriter(self.test_summary_path, tf.get_default_graph())
            global_steps = 0
            name_str = self.name + '_test_512valid.xlsx'
            excel_name = os.path.join(self.test_summary_path, name_str)
            excel, excel_activate = ops.create_excel(excel_name)
            for step in range(data.train_num // self.batch_size):
                img_an, image_org = data.GetNextBatch(self.batch_size)
                st = time.time()
                ...
                if (step + 1) % 1 == 0:
                    ...
                    second_out = sess.run(decoder_first_out,
                                          feed_dict={t_image_ph: img_an, t_org_image_ph: image_org})
                    test_out_path2 = os.path.join(self.test_out_path,epoch)
                    ops.img_save_for_all(test_out_path2, global_steps, image_size, image_org, img_an, second_out,
                                         first_out, is_train=False)
                writer.add_summary(summary, global_steps)
                global_steps += 1
            writer.close()
...
if __name__ == '__main__':
    '''路径设置'''
    name = 'vae_multi0903-04'
    path = 'log/vae_multi0903-04'
    train_summary_path = os.path.join(path, 'train_w3\summary')
    train_out_path = os.path.join(path, 'train_w3/out')

    test_summary_path = os.path.join(path, 'test_w3/summary')
    ...
    test_out_path = os.path.join(path, 'test_w3/out')
