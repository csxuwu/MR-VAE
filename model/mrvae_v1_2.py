    ...
    with tf.variable_scope(name):
        with slim.arg_scope([slim.separable_conv2d],
            ...
            with slim.arg_scope([slim.max_pool2d],
                ...
                # 卷积操作，参考xception
                conv_inputs = tf.nn.relu(inputs)
                conv = slim.separable_conv2d(inputs=conv_inputs, num_outputs=num_outputs, depth_multiplier=1,scope=scope + '_' + name + '_conv1')
                ...
                if is_pool:
                    ...
                    skip = slim.conv2d(inputs=inputs, num_outputs=num_outputs, kernel_size=1, stride=2, padding='SAME',scope=scope + '_' + name + '_skip_conv')
                    conv_out = tf.add(max_pool, skip)
                    print(name + ':{}'.format(conv.get_shape()))
                    print(name + '_max_pool:{}'.format(conv_out.get_shape()))
                    return conv, conv_out
                else:
                    print(name + ':{}'.format(conv.get_shape()))
                    return conv
...
def encoder(inputs, scope):
    ...
    with tf.variable_scope('encoder'):
        ...
        conv1, max_pool1 = encoder_block(name='downsampling1', inputs=inputs, num_outputs=64, scope=scope)
        conv2, max_pool2 = encoder_block(name='downsampling2', inputs=max_pool1, num_outputs=128, scope=scope)
        conv3, max_pool3 = encoder_block(name='downsampling3', inputs=max_pool2, num_outputs=256, scope=scope)
        conv4, max_pool4 = encoder_block(name='downsampling4', inputs=max_pool3, num_outputs=512, scope=scope)
        conv51 = encoder_block(name='z_mean', inputs=max_pool4, num_outputs=cfg_v1_2.latent_dim, is_pool=False, scope=scope)  # 最后一层，计算均值，不需要池化
        conv52 = encoder_block(name='z_variance', inputs=max_pool4, num_outputs=cfg_v1_2.latent_dim, is_pool=False, scope=scope)  # 最后一层，计算方差，不需要池化
...
def decoder_block(name, inputs, num_outputs, num_outputs_up=None, en_conv=None, is_transpose=False,
    ...
    # ResNeXt_conv = ResNeXt_Unit_and_SE_ResNeXt_Unit(num_outputs=num_outputs, transform_groups=transform_groups,
    #                                                 transform_groups_num_outputs=4, kernel_size=3, stride=1)
    with tf.variable_scope(name):
        with slim.arg_scope([slim.separable_conv2d],
                            kernel_size=3,
                            weights_initializer=tf.truncated_normal_initializer(stddev=0.01),
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                # 卷积操作
                conv_input = tf.nn.relu(inputs)
                conv = slim.separable_conv2d(inputs=conv_input, num_outputs=num_outputs, depth_multiplier=1, scope=name + '_conv1')
                conv = tf.nn.relu(conv)
                conv = slim.separable_conv2d(inputs=conv, num_outputs=num_outputs, depth_multiplier=1, scope=name + '_conv2')
                skip_input = slim.conv2d(inputs=inputs,num_outputs=num_outputs,kernel_size=1,stride=1)
                conv = tf.add(conv,skip_input)
                # 上采样操作
...
def GR(inputs, conv1, conv2, conv3, conv4, up_w, up_h):
    ...
    with tf.variable_scope('decoder_first'):
        with slim.arg_scope([slim.conv2d],
            ...
            up_w *= 2
            up_h *= 2
            upsampling1 = decoder_block(name='upsampling1', inputs=inputs, num_outputs=cfg_v1_2.latent_dim,
...
def residual_block(no, inputs, inputs64=None):
    ...
    :return:
    '''
    ResNeXt = ResNeXt_Unit_and_SE_ResNeXt_Unit(num_outputs=64, transform_groups=8, transform_groups_num_outputs=4,
                                               kernel_size=3, stride=1)  # 残差块的输出通道是固定的
    ...
    with tf.variable_scope(name):
        with slim.arg_scope([slim.separable_conv2d],
                            ...
                            padding='SAME',
                            activation_fn=tf.nn.relu):  # BN后再使用激活函数
            residual1_conv1 = slim.separable_conv2d(inputs=inputs, num_outputs=64, depth_multiplier=1)
            # 第一个卷积模块：卷积 + BN + PReLU
            ...
            residual1_conv2 = slim.separable_conv2d(inputs=residual1_conv1_PRelu, num_outputs=64, depth_multiplier=1)
...
def mrvae():
    t_image_ph = tf.placeholder(name='train_images', dtype=tf.float32,
                                shape=[cfg_v1_2.batch_size, cfg_v1_2.image_size, cfg_v1_2.image_size, 3])
    ...
    #                                 shape=[None, None, None,3])
    # 搭建mrvae
    with tf.name_scope(cfg_v1_2.name):
    ...
    with tf.name_scope('Loss'):
        ...
        train_op = slim.train.AdamOptimizer(cfg_v1_2.lr).minimize(vae_loss)
