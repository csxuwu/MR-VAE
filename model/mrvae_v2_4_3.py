...
from choice import cfg_v2_4 as cfg


'''
    原始的mrvae07 的结构，现在添加对每个卷积模块特征图的输出
    验证损失函数的作用,只有KL散度+第二阶段损失
'''
...
def mrvae():
    ...
    with tf.name_scope('Loss'):
        # L2 = tf.reduce_mean(tf.reduce_sum(tf.square(decoder_first_out - t_org_image_ph), reduction_indices=[1]))        # 第一阶段
        L1 = tf.reduce_mean(tf.reduce_sum(tf.abs(t_org_image_ph - decoder_second_out), reduction_indices=[1]))          # 第二阶段
        kl_loss = tf.reduce_mean(-0.5 * tf.reduce_sum(1 + z_variance - tf.square(z_mean) - tf.exp(z_variance), 1))      # KL散度
        # content_loss = tf.reduce_mean(tf.reduce_sum(tf.square(z - z1), reduction_indices=[1]))
        # vae_loss = (L1 + kl_loss + content_loss)
        # vae_loss = (10 * L2 + kl_loss + content_loss)
        # vae_loss = (10 * L2 + L1 + kl_loss)
        vae_loss = ( L1 + kl_loss)      # 只有L1 ,KL损失
