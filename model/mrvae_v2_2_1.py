...
def encoder(inputs, scope):
    ...
    with tf.variable_scope('encoder'):
        ...
        conv51 = encoder_block(name='z_mean', inputs=max_pool4, num_outputs=cfg_v2_2.latent_dim,is_pool=False)      # 最后一层，计算均值，不需要池化
        conv52 = encoder_block(name='z_variance', inputs=max_pool4, num_outputs=cfg_v2_2.latent_dim,is_pool=False)  # 最后一层，计算方差，不需要池化
...
def GR(inputs, conv1, conv2, conv3, conv4, up_w, up_h):
    ...
    with tf.variable_scope('decoder_first'):
        with slim.arg_scope([slim.conv2d],
            ...
            up_w *= 2
            up_h *= 2
            upsampling1 = decoder_block(name='upsampling1', inputs=inputs, num_outputs=cfg_v2_2.latent_dim,
