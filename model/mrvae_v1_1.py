...
def encoder_block(name, inputs, num_outputs, is_pool=True, scope=''):
    '''
    编码模块，采用可分离卷积，返回池化之前的卷积输出，用于跳跃传递信息，和池化之后的输出，用于下一层编码
    :param name:
    :param inputs:
    :param num_outputs:
    :param is_pool:
...
def encoder(inputs, scope):
    ...
    with tf.variable_scope('encoder'):
        ...
        conv51 = encoder_block(name='z_mean', inputs=max_pool4, num_outputs=cfg_v1_1.latent_dim, is_pool=False,scope=scope)  # 最后一层，计算均值，不需要池化
        conv52 = encoder_block(name='z_variance', inputs=max_pool4, num_outputs=cfg_v1_1.latent_dim, is_pool=False,scope=scope)  # 最后一层，计算方差，不需要池化
...
def GR(inputs, conv1, conv2, conv3, conv4, up_w, up_h):
    ...
    with tf.variable_scope('decoder_first'):
        with slim.arg_scope([slim.conv2d],
            ...
            up_w *= 2
            up_h *= 2
            upsampling1 = decoder_block(name='upsampling1', inputs=inputs, num_outputs=cfg_v1_1.latent_dim,
...
def mrvae():
    t_image_ph = tf.placeholder(name='train_images', dtype=tf.float32,
                                shape=[cfg_v1_1.batch_size, cfg_v1_1.image_size, cfg_v1_1.image_size, 3])
    ...
    #                                 shape=[None, None, None,3])
    # 搭建mrvae
    with tf.name_scope(cfg_v1_1.name):
    ...
    with tf.name_scope('Loss'):
        ...
        train_op = slim.train.AdamOptimizer(cfg_v1_1.lr).minimize(vae_loss)
