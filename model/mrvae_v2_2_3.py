...
'''
    原始的mrvae07 的结构，现在添加对每个卷积模块特征图的输出,采用步长为2的卷积替代池化
'''
    ...
    with tf.variable_scope(name,reuse=tf.AUTO_REUSE):
        with slim.arg_scope([slim.conv2d],
            ...
            with slim.arg_scope([slim.max_pool2d],
                ...
                '''下采样操作'''
                if is_pool:
                    down_sampleing = slim.conv2d(inputs=conv2,num_outputs=num_outputs,stride=2,padding='SAME')
                    print(name + ':{}'.format(conv2.get_shape()))
                    print(name + '_down_sampleing:{}'.format(down_sampleing.get_shape()))
                    return conv2, down_sampleing
                else:
                    print(name + ':{}'.format(conv2.get_shape()))
                    return conv2
...
def GR(inputs, conv1, conv2, conv3, conv4, up_w, up_h):
    ...
    with tf.variable_scope('decoder_first'):
        with slim.arg_scope([slim.conv2d],
            ...
            '''上采样'''
            up_w *= 2; up_h *= 2
            GR_upsampling1 = decoder_block(name='upsampling1', inputs=inputs, num_outputs=cfg.latent_dim,num_outputs_up=512, is_resize=True, img_width=up_w, img_height=up_h)
...
def mrvae():
    ...
    with tf.name_scope('Loss'):
        ...
        global_step, train_step  = dk.set_optimizer(lr_range=cfg.lr_range,num_batches_per_epoch=cfg.lr_decay_batches,loss=vae_loss)
    ...
    return t_image_ph, t_org_image_ph, decoder_first_out, decoder_second_out,psnr_GR, psnr_DR, ssim_GR, ssim_DR, global_step, train_step, vae_loss
