...
from choice import mrvae_v2_2_2
from choice import mrvae_v2_2_3
...
def train():
    with tf.Session() as sess:
        t_image_ph, t_org_image_ph, decoder_GR, decoder_DR, psnr_GR, psnr_DR, ssim_GR, ssim_DR, global_steps, train_step, vae_loss = mrvae()
        data = PairDataSet_v2.ListDataSet(path=cfg.train_dataset_path, dataname="train", img_height=512,
                                          img_width=512)
        ...
        # model_saver, start_opoch, start_step = dk.restore_model_for_train(sess=sess, ckpt=cfg.ckpt,
            ...
            for step in range(n_batch_train):
                ...
                if global_step % 100 == 0:
                    loss_value, psnr_GR2, psnr_DR2, ssim_GR2, ssim_DR2 = sess.run(
                        ...
                        feed_dict={t_image_ph: img_an, t_org_image_ph: image_org})
                    st = time.time()
                    second_out = sess.run(decoder_DR, feed_dict={t_image_ph: img_an, t_org_image_ph: image_org})
                    ...
                    ops.data_output(excel_active=excel_active, global_steps=total_steps, step=global_step,
                                    loss_value=loss_value, psnr_first=psnr_GR2, psnr_second=psnr_DR2,
                                    ssim_first=ssim_GR2,
                                    ssim_second=ssim_DR2, Time=run_time, epoch=cfg.epoch, ep=ep)
                    excel.save(excel_name)
                    print('已存储第 {} steps的实验数据到excel:{}\n'.format(global_step, excel_name))
                    # 图像数据
                    ops.img_save_for_all(out_path=cfg.train_out_path, global_steps=global_step, image_size=cfg.image_size,
                                         image_org=image_org, img_an=img_an, first_out=first_out,
