...
from choice import mrvae_v2_3_1
...
def train():
    ...
    t_image_ph, t_org_image_ph, decoder_GR, psnr_GR, ssim_GR, global_steps, train_step, lr, vae_loss = mrvae()
        ...
        for ep in range(cfg.epoch):
            for step in range(n_batch_train):
                ...
                if global_step % 100 == 0:
                    loss_value, psnr_GR2, ssim_GR2, lr2 = sess.run([vae_loss, psnr_GR,  ssim_GR, lr],
                                                                   feed_dict={t_image_ph: img_an, t_org_image_ph: image_org})
                    st = time.time()
                    ...
                    ops.data_output(excel_active=excel_active, global_steps=total_steps, step=global_step,
                                    loss_value=loss_value, psnr_first=psnr_GR2, psnr_second=0,ssim_first=ssim_GR2,
                                    ssim_second=0, Time=run_time, epoch=cfg.epoch, ep=ep, lr=lr2)
                    excel.save(excel_name)
                    print('已存储第 {} steps的实验数据到excel:{}\n'.format(global_step, excel_name))
                    # 图像数据
                    # print(first_out)
                    ops.img_save_for_GR(out_path=cfg.train_out_path, global_steps=global_step,
                                         image_size=cfg.image_size,image_org=image_org, img_an=img_an, first_out=first_out)
                # =========================== end ===========================
